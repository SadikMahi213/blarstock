...
class ProcessController extends Controller
{
    ...
    public static function process($deposit)
    {
        ...
        $val['SUGGESTED_MEMO']       = auth()->user() ? auth()->user()->username : $deposit->donation_sender?->name ?? '';
    ...
    public function ipn()
    {
        ...
        if ($hash == $hash2) {
            ...
            if ($_POST['PAYEE_ACCOUNT'] == $pmAcc->wallet_id && $unit == $deposit->method_currency && $amo == round($deposit->final_amo,2) && $deposit->status == ManageStatus::PAYMENT_INITIATE) {
                //Update User Data
