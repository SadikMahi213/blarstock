...
class ProcessController extends Controller
{
    ...
    {   
        ...
        $val['name']            = auth()->user() ? auth()->user()->username : $deposit->donation_sender?->name ?? '';
        $val['description']     = "Payment By Razorpay";
        $val['image']           = getImage(getFilePath('logoFavicon').'/logo_dark.png');
        $val['prefill.name']    = auth()->user() ? auth()->user()->firstname . ' ' . auth()->user()->lastname : $deposit->donation_sender?->name ?? '';
        $val['prefill.email']   = auth()->user() ? auth()->user()->email : $deposit->donation_sender?->email ?? '';
        $val['prefill.contact'] = auth()->user() ? auth()->user()->mobile : $deposit->donation_sender?->mobile ?? '';
