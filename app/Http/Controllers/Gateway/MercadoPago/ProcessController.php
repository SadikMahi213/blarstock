...
class ProcessController extends Controller
{
	public static function process($deposit)
    {
     ...
     $curl            = curl_init();
     $userName        = auth()->user() ? auth()->user()->username : $deposit->donation_sender?->name ?? '';
     $userEmail       = auth()->user() ? auth()->user()->email : $deposit->donation_sender?->email ?? '';
        $preferenceData = [
            'items' => [
                [
                    'id'          => $deposit->trx,
                    'title'       => 'Deposit',
                    'description' => 'Deposit from ' . $userName,
                    'quantity'    => 1,
        ...
        if (array_key_exists('init_point', $result)) {
            $send['redirect']     = true;
